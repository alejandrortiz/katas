...
Las cartas son una cadena de texto que incluyen regalos y 
paréntesis ().

Para saber si una carta es válida ✅, debes comprobar que 
los paréntesis cierran correctamente y que, además, no vayan 
vacíos.

¡Pero ojo! Porque el Grinch ha dejado llaves { y corchetes [ 
dentro de los paréntesis que hacen que no sean válidas. Por 
suerte sólo los ha dejado en medio de los paréntesis...
...
si es válida y false si no lo es. ¡Y acaba con la travesura del Grinch
...
function isValid(letter) {
    ...
    const between_parentesis = letter.match(/(\()(.+?)(\))/);

    if (!between_parentesis) {
        return false;
    }

    return /[\{|\[]/.test(between_parentesis.join("")) === false;
}

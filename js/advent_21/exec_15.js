...
Nos ha pasado algunos ejemplos de cómo debería ser nuestra función y algunos 
resultados:
...
Lo importante: recorrer el array de izquierda a derecha para ver que la 
subida es siempre estricta, detectar el punto más alto y entonces ver que 
la bajada es estricta hacia abajo...
...
function checkSledJump(heights) {
    ...
    for (let i = 0; i < heights.length; i++) {
        ...
        const down = path[path.length - 1] - curr;
        ...
        if (curr + step_down !== path[path.length - 1]) {
            console.log(`${curr} < [${i}] -- ${curr + step_down} !== ${path[path.length - 1]}`);
            return false;
        }
    ...
    return null === step_down ? false : true;
}

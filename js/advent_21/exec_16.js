...
contiene información sobre unos números que le puede hacer 
...
¡Ojo! Si encuentras un símbolo que no entendemos, mejor que 
devolvamos un NaN:
...
function decodeNumbers(numbers) {
    const translator = { ".": 1, ",": 5, ":": 10, ";": 50, "!": 100 };

    const result = numbers
        .split("")
        .reduce(
            (accum, number, index) => {
                if (!translator.hasOwnProperty(number)) return Number.NaN;

                const operation = translator[number] < translator[numbers[index + 1]] ? '-' : '+';

                return `${accum} ${operation} ${translator[number]}`
            },
            ''
        );
